...
def read_avg_data(input_path, x_col, y_col_or_list, title, x_label, y_label,num_avg):
    ...
    set_font()
    return output_file
def plot_multi_y(input_path, x_col, y_cols, title, x_label, y_label, legend_names,num_avg):
...
if __name__ == '__main__':
    ...
    num_avg=10
    # RNN-simple
    common_title="RNN-simple(转换阈值0.6)"
    input_path="OutputExcelQuality_RNN_TOP_K_1_all_windows_256_ACCURACY_THRESHOLD_0.95_INCREMENTAL.xlsx"
    ...
    x_col="episodes"
    y_cols=["F1","recall","accuracy"]
    x_label="迭代次数"
    y_label="分数"
    legend_names=["F1得分","召回率(recall)","准确率(accuracy)"]
    ...
    # 多列绘图
    plot_multi_y(input_path, x_col, y_cols, common_title, x_label, y_label, legend_names,num_avg)