...
from apm.plot.plot_data import set_font, plot_curve


def average_every_rows(num_avg,input_file, output_file, sheet_name='Sheet1', column='A'):
    """
    ...
    """
    # 读取Excel数据
    df = pd.read_excel(input_file, usecols=column, header=None)
    ...
    # 将结果保存到新Excel
    result_df = pd.DataFrame({"平均值": averages})
    result_df.to_excel(output_file, index=False, header=["命中率"])
